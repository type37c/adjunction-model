...
「物理的意味的随伴モデル」は、この保留構造を実装し、そのダイナミクスを検証するためのアーキテクチャです。世界（Shape）とエージェントの行動（Action）の間の構造的関係を記述する「随伴構造」は、知性が参照する安定した状態（川床）に過ぎません。真の知性は、この随伴が機能不全に陥った際（coherence breakdown）に発動し、新しい随伴を構築する「保留構造」の働きによって顕現します。
