...
## 根本原則：知性の本体は「保留構造」にある

**最重要:** このプロジェクトの真の目的は、単に誤差を最小化するAIを作ることではありません。我々が実装しようとしているのは、**知性の本質**そのものです。そして、その本質は以下の洞察に集約されます。

> **随伴構造（F⊣G）そのものが知性なのではなく、随伴が壊れた（coherence breakdown）ときに、それを修復しようとする『保留構造』の動きこそが知性の本質である。**

この原則は、すべての設計判断と実装に優先します。具体的には、以下のことを意味します。

-   **随伴は「目的」ではなく「観測対象」である:** 我々のモデルは、随伴構造が成立している安定状態（coherence）と、それが崩壊した状態（breakdown）を区別できなければなりません。
-   **Coherence Signalは「トリガー」である:** `distance(s, G(F(s)))` は、最小化すべき単なる損失ではありません。これは、モデルが「学習モード」から「思考（保留）モード」へ移行するためのトリガー信号です。
-   **創造性は「修復プロセス」である:** `coherence breakdown` が起きたとき、モデルは既存の随伴を破棄し、新しい随伴（F', G'）をシミュレーションし、coherenceを回復させる組み合わせを探します。この「新しい構造を探索するダイナミクス」こそが、我々が実装すべき創造性の本質です。
