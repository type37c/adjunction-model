...
### 5. Priority計算の再考察：創発への回帰

- **問題**: `Priority = coherence × uncertainty × valence` という計算式は、3つの軸の質的な違い（特にvalenceの時間性）を平坦化し、「どう使うか」まで設計してしまう過剰設計であった。
- **教訓**: 創発を促すためには、coherenceとuncertaintyを「観測」として、valenceを「記憶」としてAgent Cに直接渡し、それらの使い方（注意の配分）自体をAgent Cに学習させるべきである。これにより、Priority計算モジュールは不要になる可能性がある。
- **参照**: この洞察の詳細は `docs/theory/priority_and_valence_reconsidered.md` にて詳述されている。今後のAgent Cの再設計は、この文書の提案に基づいて行われるべきである。
