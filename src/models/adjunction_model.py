"""
Adjunction Model: F_C ⊣ G_C with Agent C (Intrinsic Motivation)

This integrates Agent Layer C (with intrinsic reward-based valence) with Functors F and G,
...
Key features:
- Valence updated by intrinsic rewards (curiosity + competence + novelty)
- Three-phase training process (pretrain, slack preservation, fine-tuning)

The context vector from C includes valence, modulating F and G through FiLM.
...
class AdjunctionModel(nn.Module):
    """
    Full Adjunction Model: F_C ⊣ G_C with Agent C (Intrinsic Motivation).
    
    This integrates:
    - Agent Layer C (RSSM + Priority + Intrinsic Reward-based Valence)
    ...
    def __init__(
        ...
        # Agent Layer C (with intrinsic motivation)
        self.agent_c = AgentC(
