"""
Conditional Adjunction Model v4: F_C ⊣ G_C with Agent C v4 (Intrinsic Motivation)

This integrates Agent Layer C v4 (with intrinsic reward-based valence) with Functors F and G,
...
- v3: Valence updated by coherence changes
- v4: Valence updated by intrinsic rewards (curiosity + competence + novelty)

The context vector from C v4 includes valence, modulating F and G through FiLM.
This gives Agent C 'purpose': to maximize intrinsic rewards.
"""
...
from src.models.agent_layer_v4 import AgentLayerC_v4
from src.models.conditional_adjunction import ConditionalFunctorF, ConditionalFunctorG


class ConditionalAdjunctionModelV4(nn.Module):
    """
    Full Conditional Adjunction Model v4: F_C ⊣ G_C with Agent Layer C v4 (Intrinsic Motivation).
    
    This integrates:
    - Agent Layer C v4 (RSSM + Priority + Intrinsic Reward-based Valence)
    - Conditional Functor F_C (FiLM-modulated)
    - Conditional Functor G_C (FiLM-modulated)
    - Spatial Coherence Signal computation
    - Purpose Space P (intrinsic motivation)
    """
    ...
    def __init__(
        ...
        # Agent Layer C v4 (with intrinsic motivation)
        self.agent_c = AgentLayerC_v4(
